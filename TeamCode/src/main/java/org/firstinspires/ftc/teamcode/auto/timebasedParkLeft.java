...
@Autonomous(group="1")
public class timebasedParkLeft extends CommandOpMode {
    ...
    public void run() {
        ...
        if (timer.seconds() < 1.8) {
            db.drive(-0.5, 0, 0); // Drive forward at half speed
        } else if (timer.seconds() < 2.8){
            db.drive(0, 0 , 0.5); // Stop the drivebase after 2 seconds
